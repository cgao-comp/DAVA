...
def train_DGMG_epoch(epoch, args, model, dataset, optimizer, scheduler, is_fast = False):
    ...
    for i in order:
        ...
        node_count = 1
        node_embedding = [Variable(torch.ones(1,args.node_embedding_size)).cuda()] 

        loss = 0
        while node_count<=graph.number_of_nodes():
            node_neighbor = graph.subgraph(list(range(node_count))).adjacency_list()  

            node_neighbor_new = graph.subgraph(list(range(node_count+1))).adjacency_list()[-1] 
